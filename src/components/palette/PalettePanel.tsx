...
import { Flower, Palette, Settings, TriangleAlert } from "lucide-react"
  ...
  return (
    <>
      ...
      {primaryColor === swatch.color && !isOverlay && (
        <div className="absolute -top-0.5 -right-0.5 flex size-3 items-center justify-center rounded-full bg-primary text-white shadow-sm sm:-top-1 sm:-right-1 sm:size-4">
          <svg
...
function PaletteSwatchItem({
  ...
  const dragData = useMemo(() => ({ color: swatch.color }), [swatch.color])
  const {
    attributes,
    listeners,
        ...
        <PopoverAnchor asChild>
          ...
          >
            <button
              type="button"
              className={cn(
                "relative flex aspect-square w-full items-center justify-center rounded-md border-2 transition-transform select-none hover:scale-105 active:scale-95",
                isUsedTab
...
export default function PalettePanel({
  ...
  const committedPixels = useEditorStore(
    (s) => s.history[s.historyIndex]?.pixels ?? s.pixels
  )
  ...
  const [pendingPaletteId, setPendingPaletteId] =
    useState<SystemPaletteId | null>(null)
  const [usedActionPopoverColor, setUsedActionPopoverColor] = useState<
    string | null
  >(null)
  const [draggedUsedColor, setDraggedUsedColor] = useState<string | null>(null)

  useEffect(() => {
    if (!usedTabFlashAt || handledUsedTabFlashAtRef.current === usedTabFlashAt)
      return
  ...
  const pendingIncompatibleColors = useMemo(() => {
    ...
    return canvasUsedColors.filter(
  ...
  useEffect(() => {
    ...
    if (!hasSelection || usedActionPopoverColor === normalizedSelectedColor)
      return
  ...
  useEffect(() => {
    ...
    if (normalizeHex(resolvedPrimaryColor) === normalizeHex(primaryColor))
      return
  ...
  const handleUsedColorDragEnd = useCallback(
    (event: DragEndEvent) => {
      const sourceColor = event.active.data.current?.color
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background shadow-sm">
      <div className="flex shrink-0 items-center justify-between gap-2 overflow-x-auto px-3 py-2 sm:px-4">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          <div className="flex h-8 items-center gap-1.5 rounded-md border border-primary/20 bg-primary/10 px-2.5 text-primary dark:bg-primary/20">
            <span className="hidden text-[10px] leading-none font-medium text-primary/60 sm:inline">
              {t("palette.current")}
            </span>
            <span className="max-w-24 truncate text-xs leading-none font-semibold">
      ...
      <div className="mt-1 flex-1 overflow-y-auto px-3 py-2 sm:px-4">
        <DndContext
        ...
        >
          <div
            key={tab}
            className="grid grid-cols-7 gap-2 py-1 sm:[grid-template-columns:repeat(auto-fill,minmax(52px,1fr))] sm:gap-3"
          >
      ...
      <PaletteManageDialog
        ...
        onPaletteChange={(paletteId) => {
          ...
          const targetColors = new Set(
            (targetPalette?.swatches ?? []).map((swatch) =>
          ...
          if (targetPalette) {
            const resolvedColor = resolvePaletteColor(
              primaryColor,
              targetPalette
            )
      ...
      <Dialog
        open={Boolean(pendingPaletteId)}
        onOpenChange={(open) => {
          if (!open) setPendingPaletteId(null)
        }}
      >
        <DialogContent className="max-w-md overflow-hidden rounded-2xl border-border/60 p-0">
                ...
                {pendingIncompatibleColors.length > 8 && (
                  <span className="inline-flex items-center rounded-full border border-border/60 bg-background px-2.5 py-1 text-xs text-muted-foreground shadow-sm">
                    {t("palette.changeConfirmMore", {
                      count:
                        pendingIncompatibleColors.length -
                        visiblePendingIncompatibleColors.length,
                    })}
                  </span>
                )}
              </div>
