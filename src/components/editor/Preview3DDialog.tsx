...
function PixelMesh({
  ...
  return (
    <instancedMesh
      ref={meshRef}
      args={[beadGeometry, undefined, count]}
      frustumCulled={false}
    >
...
function Scene({
  ...
  return (
    <>
      ...
      <directionalLight
        position={[-dist, dist * 0.5, -dist * 0.5]}
        intensity={0.7}
      />
      <PixelMesh
        entries={entries}
...
export default function Preview3DDialog({ open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[85vh] max-h-[calc(100vh-32px)] w-[calc(100vw-32px)] max-w-[980px] flex-col gap-0 overflow-hidden p-0">
            ...
            <>
              <div className="absolute top-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-2">
                <div className="flex items-center rounded-lg border bg-background/80 p-0.5 shadow-sm backdrop-blur-sm">
                  ...
                  <Button
                    variant={
                      beadShape === "roundedCube" ? "secondary" : "ghost"
                    }
                    size="sm"
                    className="h-7 gap-1.5 rounded-md px-2.5 text-xs"
                ...
                <div className="flex items-center rounded-lg border bg-background/80 p-0.5 shadow-sm backdrop-blur-sm">
                  ...
                  >
                    ...
                    {t(
                      autoRotate
                        ? "editor.preview3d.pauseRotation"
                        : "editor.preview3d.resumeRotation"
                    )}
                  </Button>
                </div>
              </div>
