...
export function useCursorOverlay(containerRef: React.RefObject<HTMLDivElement | null>) {
  const [cursorOverlay, setCursorOverlay] = useState<CursorOverlayState>({ x: 0, y: 0, visible: false });
  ...
  return { cursorOverlay, cursorPendingRef, queueCursorOverlay, updateCursorFromMouseEvent };
}
