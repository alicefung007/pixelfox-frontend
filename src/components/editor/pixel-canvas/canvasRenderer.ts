...
import type { Point, WandSelection } from "./types"

type RenderPixelCanvasParams = {
...
export const renderPixelCanvas = ({
  ...
  if (backgroundColor) {
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        ctx.fillStyle =
          (x + y) % 2 === 0
            ? CANVAS_CONFIG.CHECKER_LIGHT
            : CANVAS_CONFIG.CHECKER_DARK
  ...
  if (wandSelection) {
    ...
    for (const key of selectedKeys) {
      ...
      if (!color || normalizeHex(color) !== normalizeHex(wandSelection.color))
        continue
      const [x, y] = key.split(",").map(Number)
      ctx.strokeRect(x + glowInset, y + glowInset, glowSize, glowSize)
    }
  ...
  const bold5LineWidth =
    (CANVAS_CONFIG.BOLD_LINE_WIDTH * CANVAS_CONFIG.GRID_LINE_WIDTH) /
    effectiveScale
  const bold10LineWidth =
    (CANVAS_CONFIG.MAJOR_LINE_WIDTH * CANVAS_CONFIG.GRID_LINE_WIDTH) /
    effectiveScale
  ...
  ctx.beginPath()
  for (
    let x = CANVAS_CONFIG.GRID_INTERVAL_5;
    x <= width;
    x += CANVAS_CONFIG.GRID_INTERVAL_5
  ) {
  ...
  for (
    let y = CANVAS_CONFIG.GRID_INTERVAL_5;
    y <= height;
    y += CANVAS_CONFIG.GRID_INTERVAL_5
  ) {
  ...
  ctx.beginPath()
  for (
    let x = CANVAS_CONFIG.GRID_INTERVAL_10;
    x <= width;
    x += CANVAS_CONFIG.GRID_INTERVAL_10
  ) {
    ctx.moveTo(x, 0)
    ctx.lineTo(x, height)
  }
  for (
    let y = CANVAS_CONFIG.GRID_INTERVAL_10;
    y <= height;
    y += CANVAS_CONFIG.GRID_INTERVAL_10
  ) {
    ctx.moveTo(0, y)
    ctx.lineTo(width, y)
  }
