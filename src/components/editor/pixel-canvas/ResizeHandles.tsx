...
type CanvasScreenRect = { left: number; top: number; width: number; height: number };
...
export default function ResizeHandles({ canvasScreenRect, scale, resizeDrag, startResize }: Props) {
  const handles = buildHandles(canvasScreenRect);
  const resizePreview = resizeDrag ? buildPreview(resizeDrag, canvasScreenRect, scale) : null;
