...
export function usePointerTools(params: {
  ...
  const onMouseDown = (e: React.MouseEvent) => {
    ...
    if (currentTool === 'wand') {
      ...
      setIsDrawing(false);
      setLastCoords(null);
      handleWandSelection(getCoordinates(canvasRef.current, e, zoom, viewOffset, width, height));
      return;
    }
    strokeColorRegisteredRef.current = false;
    setIsDrawing(true);
    handleDraw(getCoordinates(canvasRef.current, e, zoom, viewOffset, width, height));
  };
  ...
  }, [isDrawing, isPanning, saveHistory, setIsDrawing, setLastCoords, strokeColorRegisteredRef]);
