...
export const usePaletteStore = create<PaletteState>()(
  persist(
    (set) => ({
      ...
      setSelectedUsedColor: (color) =>
        set({ selectedUsedColor: color ? normalizeHex(color) : null }),
      ...
      addRecentColor: (color) =>
        set((state) => {
          ...
          if (
            state.recentColors.some((c) => normalizeHex(c) === normalizedColor)
          ) {
            return {
              recentColors: [
                normalizedColor,
                ...state.recentColors.filter(
                  (c) => normalizeHex(c) !== normalizedColor
                ),
              ].slice(0, PALETTE_CONFIG.RECENT_COLORS_LIMIT),
            }
          }
          return {
            recentColors: [normalizedColor, ...state.recentColors].slice(
              0,
              PALETTE_CONFIG.RECENT_COLORS_LIMIT
            ),
          }
        }),
      addUsedColor: (color) =>
        set((state) => {
          const normalizedColor = normalizeHex(color)
          if (state.usedColors.some((c) => normalizeHex(c) === normalizedColor))
            return state
          return {
            usedColors: [normalizedColor, ...state.usedColors].slice(
              0,
              PALETTE_CONFIG.USED_COLORS_LIMIT
            ),
          }
        }),
