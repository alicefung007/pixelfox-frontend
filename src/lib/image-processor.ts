...
function rgbToLab(
  ...
  const fx =
    X / D65_Xn > 0.008856
      ? Math.cbrt(X / D65_Xn)
      : (7.787 * X) / D65_Xn + 16 / 116
  const fy =
    Y / D65_Yn > 0.008856
      ? Math.cbrt(Y / D65_Yn)
      : (7.787 * Y) / D65_Yn + 16 / 116
  const fz =
    Z / D65_Zn > 0.008856
      ? Math.cbrt(Z / D65_Zn)
      : (7.787 * Z) / D65_Zn + 16 / 116
...
function getCIEDE2000(
  ...
  const SL =
    1 +
    (0.015 * Math.pow(LMean - 50, 2)) / Math.sqrt(20 + Math.pow(LMean - 50, 2))
  const SC = 1 + 0.045 * CMeanP
  const SH = 1 + 0.015 * CMeanP * T

  const RT =
    -2 *
    Math.sqrt(CMean7 / (CMean7 + POW25_7)) *
    Math.sin(60 * Math.exp(-Math.pow((hMeanP - 275) / 25, 2)) * PI180)
...
export function createColorMatcher(palette: PaletteDefinition) {
  ...
  const labelToIndex = new Map(
    beadPalette.map((bead) => [bead.label, bead.index])
  )
...
export async function convertImageToPixelArt(
  ...
  for (let py = 0; py < pooledHeight; py++) {
    for (let px = 0; px < pooledWidth; px++) {
      ...
      const matchedBead = findNearestColor(
        targetR,
        targetG,
        targetB,
        excludeSet
      ...
      if (baseAlpha === 0) {
        ...
        while (head < queue.length) {
          ...
          for (const [dx, dy] of directions) {
            const nx = cx + dx
            const ny = cy + dy
            if (nx < 0 || nx >= pooledWidth || ny < 0 || ny >= pooledHeight)
              continue
...
function labelToIndexMap(
  beadPalette: PaletteBead[],
  label: string
): number | undefined {
